...
// node_modules/rdf-reasoner-konclude/dist/types.js
var INFERRED_GRAPH_IRI = "urn:konclude:inferred";

// node_modules/rdf-reasoner-konclude/dist/index.js
function serializeToNTriples(quads) {
  return new Promise((resolve, reject) => {
    const writer = new N3Writer({ format: "application/n-triples" });
    for (const quad2 of quads) {
      writer.addQuad(quad2.subject, quad2.predicate, quad2.object);
    }
...
function parseNTriples(ntriplesStr) {
  return new Promise((resolve, reject) => {
    const parser = new N3Parser({ format: "N-Triples" });
    const quads = [];
    parser.parse(ntriplesStr, (err, quad2) => {
      if (err)
        reject(err);
      else if (quad2)
        quads.push(quad2);
...
var RdfReasoner = class {
  ...
  _reasonOnStore(store, opts) {
    const result = this._queue.then(async () => {
      const inferredGraphNode = N3DataFactory_default.namedNode(opts?.inferredGraph ?? INFERRED_GRAPH_IRI);
      ...
      const resultStr = await this._call("getInferredNTriples");
      const inferredQuads = await parseNTriples(resultStr);
      for (const q of inferredQuads) {
        store.addQuad(N3DataFactory_default.quad(q.subject, q.predicate, q.object, inferredGraphNode));
      }
    });
  ...
  checkConsistency(input) {
    const quads = input instanceof N3Store ? input.getQuads(null, null, null, null) : input;
    const result = this._queue.then(async () => {
      const ntriplesStr = await serializeToNTriples(quads);
      await this._call("loadNTriples", ntriplesStr);
      await this._call("classify");
      return await this._call("isConsistent");
    });
    this._queue = result.then(() => {
    }, () => {
    });
